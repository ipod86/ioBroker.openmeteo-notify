...
const WarningsPanel: React.FC<Props> = ({
	native,
	onChange,
	notificationManagerInstalled,
	notificationManagerRunning,
}) => {
	...
	return (
		<Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
			{notificationManagerInstalled === false && (
				<Alert severity="warning">{I18n.t("notificationManagerMissingHint")}</Alert>
			)}
			{notificationManagerInstalled === true && notificationManagerRunning === false && (
				<Alert severity="warning">{I18n.t("notificationManagerNotRunningHint")}</Alert>
			)}
			{notificationManagerInstalled === true && notificationManagerRunning === true && (
				<Alert severity="info">{I18n.t("notificationManagerFoundHint")}</Alert>
			)}
			{/* Official warnings */}
			<Box>
				<Typography
				...
				<Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
					<FormControlLabel
						control={
							<Switch
								checked={!!native.warnOfficial}
								onChange={e => update("warnOfficial", e.target.checked)}
							/>
						}
					...
					{native.warnOfficial && (
						<Box sx={{ ml: 2, mt: 1 }}>
							<FormControlLabel
								control={
									<Switch
										checked={!!native.warnNotifyLift}
										onChange={e => update("warnNotifyLift", e.target.checked)}
									/>
								}
			...
			<Box>
				...
				<Typography
					...
					sx={{ mb: 1 }}
				>
					{I18n.t("warningsOpenMeteoHint")}
				</Typography>
				<Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
					<FormControlLabel
						control={
							<Switch
								checked={!!native.warnStorm}
								onChange={e => update("warnStorm", e.target.checked)}
							/>
						}
							...
							>
								{[
									...
									>
										...
										{o.isDefault ? ` – ${I18n.t("defaultValue")}` : ""}
									</MenuItem>
								))}
							</Select>
						</FormControl>
					...
					<FormControlLabel
						control={
							<Switch
								checked={!!native.warnThunderstorm}
								onChange={e => update("warnThunderstorm", e.target.checked)}
							/>
						}
					...
					<FormControlLabel
						control={
							<Switch
								checked={!!native.warnFrost}
								onChange={e => update("warnFrost", e.target.checked)}
							/>
						}
					...
					{native.warnFrost && (
						<TextField
							...
							helperText={
								(native.warnFrostThreshold ?? 0) < -20 || (native.warnFrostThreshold ?? 0) > 5
					...
					{(native.warnStorm || native.warnThunderstorm || native.warnFrost) && (
						<>
							<TextField
								...
								helperText={
									(native.warnLeadHours ?? 2) < 1 || (native.warnLeadHours ?? 2) > 24
