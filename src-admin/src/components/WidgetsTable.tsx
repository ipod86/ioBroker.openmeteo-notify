...
const ColorSwatch: React.FC<{ value: string; onChange: (v: string) => void; disabled?: boolean }> = ({
	value,
	onChange,
	disabled,
}) => (
					...
					>
						<Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
							<Typography variant="subtitle2">
								Widget {i + 1} — DP:{" "}
								<code>{w.locationName ? `${w.locationName}.widget.${w.id}` : "…"}</code>
							</Typography>
							<IconButton
								onClick={() => remove(i)}
								color="error"
								...
								<ToggleButtonGroup
									...
									onChange={(_, v) => {
										...
										if (v === "custom") {
											...
											const base =
												PRESETS[(w.theme as "dark" | "light") ?? "dark"] ?? PRESETS.dark;
											update(i, {
												theme: "custom",
												bgColor: w.bgColor || base.bgColor,
												textBase: w.textBase || base.textBase,
											});
										} else {
								...
								>
									...
									<Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
										...
										<Switch
											...
											onChange={e =>
												update(i, { bgColor: e.target.checked ? "transparent" : "#1e1e1e" })
											}
										/>
										<Typography
											variant="caption"
									...
									<Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
										...
										>
											<Typography
												variant="caption"
												sx={{ color: resolvedText, fontWeight: 600, whiteSpace: "nowrap" }}
											>
												Abc 123
											</Typography>
										</Box>
